...
class ProgramStore {
    ...
    @action getLessonName(id) {
        return Api('/lesson-by-id', {method: 'post', body: {'lesson_slug': id}})
            .then(action('then', (data) => {
    ...
    @action setLessonCompleted(){
        this.inProgress = true;
        return Api('/set-lesson-completed', {method: 'post', body: {'lesson_id': this.CurrLesson}})
            .then(action('then', (data) => {
                this.inProgress = false;
                this.RecomendedProgram = data;
