...
class AccessProductPopup extends Component {
    ...
    render() {
        if (this.props.viewerStore.isAfterPaymentPopup){
            {this.bodyClass(true)}
            return (
                <div className="c-popup-cover app-popup-mob" onClick={()=>{this.hidePopup()}}>
                    <div className="c-popup c-app-popup" onClick={(e)=>{e.stopPropagation()}}>
                        ...
                        <div className="app-bottom">
                            ...
                            <Link to="#" className="btn2" onClick={()=>this.props.viewerStore.showLogin()}>Inicia sesión Aquí</Link>
                        </div>
                    </div>
                </div>
            );
