...
class ContentsTable extends Component {
        ...
        if(this.props.programStore.inProgressLessons){
            ...
            return  this.props.programStore.ModuleLessons.map(item =>
                <li key={item.id} className={item.id==this.props.programStore.CurrLesson?"active":""}>
                    <LinkArr chref={"/programas/"+this.props.programStore.CurrProgram+"/modulo-"+this.props.programStore.CurrModule+"/leccion-"+item.id} click={()=>{this.props.programStore.setCurrLesson(item.id, item.name);}} className="c-ct-list-item" checked={item.is_completed}>{module_id}.{++i}. {item.name}</LinkArr>
                </li>);
        }
    }
    ...
    showModules(){
        ...
        return  this.props.programStore.ProgramModules.map(item =>
            <li key={i++}>
                <Link to={"/programas/"+this.props.programStore.CurrProgram+"/modulo-"+item.id} onClick={()=>{this.props.programStore.setCurrModule(item.id, item.name); this.props.programStore.pullModuleLessons(); }} className="c-ct-list-item">
